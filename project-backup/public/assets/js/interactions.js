(() => {
  ...
  if(section(1)) {
    ...
    async function refresh(){ $('#cloud-status').textContent='讀取中……';try{const result=await B3API.getDashboardData(); if(!Array.isArray(result.data.word_cloud)||!result.data.radar)throw new Error('請更新 Apps Script 至新版互動 API。');rows=result.data.word_cloud;const selected=$('#mood-filter').value;$('#mood-filter').replaceChildren(new Option('全部心情',''));[...new Set(rows.map(r=>r.mood))].forEach(m=>$('#mood-filter').add(new Option(m,m)));$('#mood-filter').value=[...$('#mood-filter').options].some(o=>o.value===selected)?selected:'';drawCloud();$('#cloud-status').textContent=result.mode==='local'?'本機離線資料；不同裝置尚未共用。':'已讀取本次工作坊資料。';if($('#class-radar')){const r=result.data.radar;if(r.count)radar($('#class-radar'),r.scores,`全班平均（${r.count} 位，每人最新一次）`);else $('#class-radar').textContent='尚無完整自評資料，請先完成任務 4。';}}catch(e){$('#cloud-status').textContent='讀取失敗：'+e.message;}}
    $('#mood-filter').addEventListener('change',drawCloud);$('#refresh-cloud').addEventListener('click',refresh);document.addEventListener('b3:saved',e=>{if(e.target.dataset.apiAction==='submitWordCloud')refresh();});refresh();
  }
  if(section(4)) {
