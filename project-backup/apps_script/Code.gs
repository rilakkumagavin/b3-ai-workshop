...
const lock=LockService.getScriptLock();if(!lock.tryLock(10000))b3Fail_('BUSY','請稍後再試。');try{if(!b3PublicState_().submissionsOpen)b3Fail_('ACTIVITY_PAUSED','主持人已暫停提交，請保留輸入並稍後再試。');const book=b3Book_();if(spec.fields.includes('group_id')&&(!data.group_id||!data.name)&&data.participant_id){const people=b3Rows_(book,'submitParticipant');const p=people.filter(r=>r.participant_id===data.participant_id).pop();if(p){data.group_id=data.group_id||p.group_id;if(spec.fields.includes('name'))data.name=data.name||p.name;}}
...
function b3Dashboard_(id){id=b3Workshop_(id);const book=b3Book_(),rows=[],counts={};Object.keys(B3_SHEET_CONTRACT).forEach(action=>{const entries=b3Rows_(book,action);counts[action]=entries.length;entries.forEach(d=>rows.push({action,participantId:d.participant_id||'group-'+d.group_id,timestamp:d.timestamp,data:b3FromSheet(action,d)}));});const projection=b3ProjectionData(rows);const people=new Map();rows.filter(r=>r.action==='submitParticipant').forEach(r=>people.set(r.participantId,r.data));const groups=new Map();people.forEach(d=>{const g=String(d.groupName||'未分組');groups.set(g,(groups.get(g)||0)+1);});return{session:b3PublicState_(),participantCount:people.size,groupCount:[...groups.keys()].filter(g=>g!=='未分組').length,lessonCount:counts.submitLessonMarket,groups:[...groups].map(([group,count])=>({group,count})),...b3InteractiveAggregate(rows),...projection,submissionCounts:counts,generatedAt:new Date().toISOString(),workshopId:id};}
